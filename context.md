...
The 2026-09-22 UI refinement keeps CoCreate Recommended first and makes its dialog hierarchy explicit: Funding, the three modes, cost/limits, then technical detail. The four effort tiers were subsequently moved out of the dialog into the canvas-side build-control rail, where owner changes still use the same recommendation endpoint and server-enforced spending ceiling. The interface now uses a graphite, silver, pearl, and blue-gray palette without green accents. Existing Lucide symbols receive one CSS-only liquid-metal system with slow shape deformation and moving highlights; reduced-motion preferences disable practical motion. No copied artwork, animation library, or new asset dependency was added.

The 2026-09-21 connection experience now leads with CoCreate Recommended, as explicitly requested, while making its funding/model source clear: capability-tested models come from the owner's saved API connection. **Connect your API** and **Advanced · choose my own models** expose add/edit/disconnect, optional discovery, exact manual model testing, four separate capability results, and personal-interpreter/shared-executor assignment; **View recommended setup** returns without erasing those settings. No passive UI action runs a model.
