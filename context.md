...
The 2026-09-25 account/sharing slice adds Supabase email/password signup, confirmation resend, login, recovery, authenticated password update, and safe destination preservation alongside Google PKCE. The existing project system now supports named creation, owner-only rename, folder-outline sidebar affordances, and a Share project dialog backed by email-bound editor/viewer invitations, explicit sharing permission, pending invite resend/revoke, and truthful transactional-email states. The canvas effort UI is an accessible Low/Medium/High/Extra segmented radio group while preserving the historical `light` value and inference-free future-submission behavior. The SQL migration and code are complete locally; live migration, Supabase Auth/SMTP dashboard verification, and a real Resend sender/key remain external verification work unless recorded otherwise.
