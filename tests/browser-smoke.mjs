...
try{
  ...
  const canvasEffort=await evaluate(`(()=>{const group=document.querySelector('.effort-toggle'),range=group?.querySelector('input[type="range"]'),options=[...group?.querySelectorAll('button')||[]].map(option=>({label:option.textContent,selected:option.getAttribute('aria-pressed')}));return{disabled:range?.disabled,value:range?.value,valueText:range?.getAttribute('aria-valuetext'),options}})()`);
  if(canvasEffort.options.length!==4||canvasEffort.value!=='1'||canvasEffort.valueText!=='Medium'||!canvasEffort.disabled||canvasEffort.options.map(item=>item.label).join('|')!=='Low|Medium|High|Extra')throw new Error(`AI effort toggle did not render safely while Recommended is inactive: ${JSON.stringify(canvasEffort)}`);
