...
The Vite browser client starts OAuth only with the Google provider. It derives the exact application callback from the explicitly configured app origin: production uses `https://cocreate.susan981314271.workers.dev/api/auth/callback`, while local development uses `http://localhost:5173/api/auth/callback`. Supabase returns from Google at its separate provider callback, `https://dnsapasubeoxxsgkiotw.supabase.co/auth/v1/callback`. The browser keeps PKCE sessions through `@supabase/supabase-js` automatic refresh, exchanges each callback code at most once, accepts only same-origin relative post-login destinations, and shows denied, missing, expired, or reused-code failures without exposing credentials. Missing or mismatched public configuration fails closed. Before an authenticated project request the client refreshes a token within the configured one-minute boundary; a 401 permits exactly one explicit refresh-and-retry. This is client request handling, not Next.js cookie middleware.
