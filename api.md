...
The Vite browser client starts OAuth with the Google provider and the default production redirect `https://cocreate.pages.dev/api/auth/callback`, then keeps PKCE sessions through `@supabase/supabase-js` automatic refresh. The callback must be present in Supabase's redirect allow list; `VITE_SUPABASE_OAUTH_REDIRECT_URL` supports an explicitly allow-listed environment override. Before an authenticated project request the client refreshes a token within the configured one-minute boundary; a 401 permits exactly one explicit refresh-and-retry. This is client request handling, not Next.js cookie middleware.
