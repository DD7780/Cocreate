import type{AIRate,AIRunCall,AIRunRecord,AISpecialty,AIEffort,NormalizedAIUsage,TaskComplexity}from'../src/types.js';
...
export type EffectivenessGroup={configuration:string;specialty?:AISpecialty;effort?:AIEffort;complexity:TaskComplexity;verificationPolicyVersion:string;sampleSize:number;verificationPassRate?:number;medianLatencyMs?:number;totalEstimatedCostUsd:number;costPerSuccessfulVerifiedBuildUsd?:number};
export function effectiveness(records:AIRunRecord[],minimumSamples=3):EffectivenessGroup[]{
  const groups=new Map<string,AIRunRecord[]>();for(const record of records){const configuration=[[...record.personalModels].sort().join(','),record.builderModel,record.specialty,record.effort,record.complexity,record.verificationPolicyVersion].join('|');groups.set(configuration,[...(groups.get(configuration)||[]),record])}
  return[...groups.entries()].map(([configuration,items])=>{const verified=items.filter(item=>item.verification.verified).length,costs=items.map(item=>item.usage.estimatedChargeUsd),complete=costs.every(value=>value!==undefined),latencies=items.map(item=>item.latencyMs).sort((a,b)=>a-b),enough=items.length>=minimumSamples;return{configuration,specialty:items[0].specialty,effort:items[0].effort,complexity:items[0].complexity,verificationPolicyVersion:items[0].verificationPolicyVersion,sampleSize:items.length,verificationPassRate:enough?verified/items.length:undefined,medianLatencyMs:enough?latencies[Math.floor(latencies.length/2)]:undefined,totalEstimatedCostUsd:complete?costs.reduce((n,value)=>n+value!,0):NaN,costPerSuccessfulVerifiedBuildUsd:enough&&verified&&complete?costs.reduce((n,value)=>n+value!,0)/verified:undefined}});
}
