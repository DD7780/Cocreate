...
Everyone brainstorms and co-writes in the same rich canvas. Each collaborator's evolving contribution is maintained separately as additions, modifications, and withdrawals so ideas are combined without losing authorship. The builder receives both the complete current canvas and those attributed contribution summaries, then updates one shared Product. **Build now** first incorporates every pending contribution, then runs that single builder. A failed generation keeps the last successful Product visible and reports the error.
