...
The spending limit is a total safety ceiling; it does not increase a model call's output-token allowance. New Developer presets use 8K / 12K / 20K / 32K executor output allowances for Light / Medium / High / Extra, while Advanced defaults to Medium's 12K. If a structured response truncates, CoCreate makes one already-budgeted compact retry and retains usage from both attempts. If that also truncates, choose a higher effort or a model with a larger output limit.
