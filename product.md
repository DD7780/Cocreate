...
- Present Light, Medium, High, and Extra in one compact ChatGPT-style AI effort picker beside the canvas. The owner can use it with either Recommended or Advanced assignments; Advanced keeps its chosen models while the level changes bounded input/output and repair allowances for future frozen submissions.
...
CoCreate's hosted product begins with Google sign-in and a private, searchable project list. One project maps to one existing room/workflow. Creating, opening, switching, renaming, archiving, or inviting never invokes a model. Owner, editor, and viewer memberships are explicit; a URL alone grants nothing. The canvas keeps submission-first execution, the existing Developer/Analyst/Researcher availability, AI effort, Alt+X, and the last working artifact.
