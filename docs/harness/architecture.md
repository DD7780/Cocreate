...
The spending limit is an aggregate safety ceiling and never becomes a per-call token control. Version `2026-09-22.v3` gives new Developer setups practical bounded executor outputs of 8K / 12K / 20K / 32K by effort; Advanced's default is the same 12K as Medium. A structured response that reaches its output allowance receives one compact retry inside the already-disclosed two-call schema-repair bound. Provider-reported usage from both attempts is retained. A second truncation fails explicitly without increasing the frozen effort allowance or silently switching models/providers.
