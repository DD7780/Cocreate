...
The provider HTTP boundary now creates a unique record for every physical request attempt. Its dispatch intent precedes the fetch and its reconciliation records provider/model/configuration, purpose, retry reason, timestamps, provider request ID when supplied, normalized usage, and outcome without prompts, document contents, or credentials. Transport retries and structured-output repairs therefore cannot disappear inside one logical run total. Room-scoped discovery and capability checks persist these records and expose their aggregate separately as setup usage. Extending this same durable record context to interpretation and executor dispatch, awaiting hosted persistence before network dispatch, and reconciling orphaned intents after restart remain explicit unfinished criteria.
